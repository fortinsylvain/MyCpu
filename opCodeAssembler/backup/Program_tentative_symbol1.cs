...
//        }
        ...
        static void Main(string[] args)
        {
            ...
            dataList.Add(new InstrTable { StringValue = "ORG", OpCode = 0, NbByte = 0, Offset = 0 });  // ORG/****H    Set the address where to assemble
            dataList.Add(new InstrTable { StringValue = "DB", OpCode = 0, NbByte = 0, Offset = 3 });  // DB **H   Define Byte in EEPROM Memory
            dataList.Add(new InstrTable { StringValue = "JSR", OpCode = 0x06, NbByte = 2, Offset = 4 });  // JSR ****H    Jump to SubRoutine
            ...
            dataList.Add(new InstrTable { StringValue = "JRA", OpCode = 0x0C, NbByte = 1, Offset = 4 });  // JRA **H      Unconditional relative jump
            ...
            dataList.Add(new InstrTable { StringValue = "ADDA", OpCode = 0x29, NbByte = 2, Offset = 5 });  // ADDA ****H   Add Byte from Address into REG A Carry update
            dataList.Add(new InstrTable { StringValue = "LDA", OpCode = 0x2A, NbByte = 2, Offset = 4 });  // LDA ****H    Load Byte from Address into REG A
            dataList.Add(new InstrTable { StringValue = "JNE", OpCode = 0x2B, NbByte = 2, Offset = 4 });  // JNE ****H    JUMP IF NOT EQUAL (E=0)
            dataList.Add(new InstrTable { StringValue = "JEQH", OpCode = 0x2C, NbByte = 2, Offset = 4 });  // JEQ ****H    JUMP IF EQUAL (E=1)
            dataList.Add(new InstrTable { StringValue = "CMPA #", OpCode = 0x2D, NbByte = 1, Offset = 6 });  // CMPA #**H    COMPARE REGISTER A WITH IMMEDIATE BYTE, E=1 equal, E=0 different
            dataList.Add(new InstrTable { StringValue = "ADCA #", OpCode = 0x2E, NbByte = 1, Offset = 6 });  // ADCA #**H    REG A = REG A + IMMEDIATE BYTE + CARRY (C), Carry C Updated
            dataList.Add(new InstrTable { StringValue = "ADDA #", OpCode = 0x2F, NbByte = 1, Offset = 6 });  // ADDA #**H    ADD IMMEDIATE BYTE VALUE TO REGISTER A  C UPDATED
            dataList.Add(new InstrTable { StringValue = "LDA #", OpCode = 0x30, NbByte = 1, Offset = 5 });  // LDA #**H     LOAD IMMEDIATE VALUE IN REGISTER A
            dataList.Add(new InstrTable { StringValue = "STA", OpCode = 0x31, NbByte = 2, Offset = 4 });  // STA ****H    STORE REG.A TO ADDRESSE
            dataList.Add(new InstrTable { StringValue = "JMP", OpCode = 0x32, NbByte = 2, Offset = 4 });  // JMP ****H    JUMP INCONDITIONAL TO ADDRESS
            dataList.Add(new InstrTable { StringValue = "ANDA #", OpCode = 0x33, NbByte = 1, Offset = 6 });  // ANDA #**H    REGISTER A AND LOGICAL WITH IMMEDIATE BYTE
            dataList.Add(new InstrTable { StringValue = "ORA #", OpCode = 0x34, NbByte = 1, Offset = 5 });  // ORA #**H     LOGICAL OR BETWEEN REG A AND IMMEDIATE BYTE
            dataList.Add(new InstrTable { StringValue = "XORA #", OpCode = 0x35, NbByte = 1, Offset = 6 });  // XORA #**H    EXCLUSIVE OR BETWEEN REG A AND IMMEDIATE BYTE
            ...
            dataList.Add(new InstrTable { StringValue = "LDX #", OpCode = 0x38, NbByte = 2, Offset = 5 });  // LDX #****H   Load X Register with 16 bits immediate value
                ...
                using (StreamWriter lstFile = File.CreateText(Path.Combine(sRepositoryPath, baseFileName + ".lst")))
                {
                    ...
                    while (!inputFile.EndOfStream)
                    {
                        ...
                        else // Process the line
                        {
                            ...
                            string sMnemonic = ExtractSubstringFromPosition(sLine, iFirstCharacterIndex);
                            iLenghtOfMnemonic = sMnemonic.Length;
                            bool bFound = false;
                            ...
                            foreach (InstrTable data in dataList)
                            {
                                ...
                                if (iLenghtOfMnemonic == iCodeLength)   // compare only if string are the same size
                                {
                            ...
                            if (bFound)
                            {
                                ...
                                if (iIndexTable != 0)  // Only if not an ORG
                                {
                                    if (iPass == 2)
                                    {
                                        ...
                                        // get string after mnemonic length
                                        int iStartOfDataString = iFirstCharacterIndex + dataList[iIndexTable].StringValue.Length;
                                        string sDataStringAfterMnemonic = ExtractSubstringFromPosition(sLine, iStartOfDataString);
        ...
        private static string ExtractSubstringFromPosition(string input, int start)
        {
            int spaceOrLineFeedIndex = input.IndexOfAny(new char[] { ' ', '\n' }, start);
            if (spaceOrLineFeedIndex == -1)
            {
                return input.Substring(start);
            }
            return input.Substring(start, spaceOrLineFeedIndex - start);
        }
